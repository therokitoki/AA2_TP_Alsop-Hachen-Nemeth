...
class NNAgent(Agent):
    ...
    def act(self, state):
        ...
        b_vec[b] = 1
        c_vec[c] = 1
        vector = np.concatenate([a_vec, b_vec, c_vec])
        # change vector to (None, 63)
        vector_ok = np.expand_dims(vector, axis=0)

        q_values = self.model(vector_ok)
